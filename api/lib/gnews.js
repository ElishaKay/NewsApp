...
const topic = async (topicName, {country = 'israel', language = 'hl', n = 10}={}) => {
  if (!TOPICS.includes(topicName)) throw 'Invalid topic name. See list of topics for valid names.';
  const url = TOPICS_RSS + topicName + '?' + fillCountryLangParams('ISRAEL', language.toLowerCase());
  console.log(await getRss(url)).items.slice(0, Math.max(0, n));
}
